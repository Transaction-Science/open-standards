//! JCB J/Secure Directory Server adapter.
//!
//! JCB-specific notes
//! ------------------
//! - ARes `eci` for JCB: `"05"` / `"06"` (Visa-aligned coding).
...
impl Default for JcbDs {
    fn default() -> Self {
        Self {
            acs_reference_number: "3DS_LOA_ACS_JCBN_020300_00001".into(),
            stub_trans_status: "Y".into(),
            stub_cavv: "JCBCAVVAAAAAABBkgkkR=".into(),
        }
    }
}

#[async_trait]
impl DirectoryServer for JcbDs {
    async fn version_check(&self, _card_range_pan: &str) -> Result<VersionCheckResponse> {
        Ok(VersionCheckResponse {
            ...
            three_ds_method_url: Some("https://3ds.jcb.example/method".into()),
        })
    }
    ...
    async fn auth_request(&self, areq: &AReq) -> Result<ARes> {
        Ok(ARes {
            ...
            ds_trans_id: "ds-jcb-00000001".into(),
            acs_trans_id: "acs-jcb-00000001".into(),
            trans_status: self.stub_trans_status.clone(),
            acs_reference_number: self.acs_reference_number.clone(),
            acs_operator_id: "jcb-ds".into(),
            acs_url: None,
            acs_challenge_mandated: None,
            authentication_value: Some(self.stub_cavv.clone()),
            eci: Some(
                eci_for(crate::directory_server::DsRoute::Jcb, &self.stub_trans_status)
                    .to_owned(),
            ),
