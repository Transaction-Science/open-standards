...
/// Default endpoint for Jina v1.
pub const DEFAULT_ENDPOINT: &str = "https://api.jina.ai/v1/embeddings";
...
impl JinaEmbedder {
    /// Build a `JinaEmbedder` for `model`.
    ...
    async fn embed(&self, texts: &[&str]) -> EmbeddingResult<Vec<Vec<f32>>> {
        ...
        loop {
            ...
            if status.as_u16() == 429 {
                ...
                warn!(attempt, wait, "jina 429; backing off");
...
mod tests {
    ...
    fn dimensions_per_model() {
        assert_eq!(
            JinaEmbedder::new("k", "jina-embeddings-v3").expect("ok").dimensions(),
            1024
        );
        assert_eq!(
            JinaEmbedder::new("k", "jina-embeddings-v2-base-code").expect("ok").dimensions(),
            768
        );
    }
