//! Together.ai chat-completions backend (OpenAI-compatible schema).
...
impl TogetherBackend {
    ...
    async fn try_infer(&self, q: &Query) -> VendorResult<Response> {
        tracing::debug!(
            target: "together.infer",
            model = %self.model,
            api_key = field::Empty,
            "dispatching together inference"
        );
