//! Mistral chat-completions backend (OpenAI-compatible schema).
...
impl MistralBackend {
    /// Construct with an API key and target model.
    ...
    async fn try_infer(&self, q: &Query) -> VendorResult<Response> {
        tracing::debug!(
            target: "mistral.infer",
            model = %self.model,
            api_key = field::Empty,
            "dispatching mistral inference"
        );
